...
export default {
    name: 'ecchi',
    description: 'Get ecchi image',
    groupOnly: true,
    execute: async (sock, msg, args, context) => {
        // Check if NSFW is enabled (for NSFW commands)
        const isNSFW = ['hentai', 'ecchi'].includes('ecchi');
        ...
        try {
            ...
            const response = await axios.get(`https://api.waifu.pics/${endpoint}/ecchi`);
            ...
            console.error('ecchi error:', error);
            await sock.sendMessage(context.from, {
