...
export default {
    name: 'oppai',
    description: 'Get oppai image',
    groupOnly: true,
    execute: async (sock, msg, args, context) => {
        // Check if NSFW is enabled (for NSFW commands)
        const isNSFW = ['hentai', 'ecchi'].includes('oppai');
        ...
        try {
            ...
            const response = await axios.get(`https://api.waifu.pics/${endpoint}/oppai`);
            ...
            console.error('oppai error:', error);
            await sock.sendMessage(context.from, {
